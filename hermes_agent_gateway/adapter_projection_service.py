...
class AdapterProjectionService:
    def gateway_descriptor(self) -> GatewayAgentDescriptor:
        return build_gateway_agent_descriptor()

    def normalize_message_envelope(self, payload: Mapping[str, Any]) -> GatewayMessageEnvelope:
        return normalize_message_envelope(payload)

    def normalize_adapter_message_envelope(
        self,
        payload: Mapping[str, Any],
        *,
        actor_source: str | None = None,
    ) -> GatewayMessageEnvelope:
        return normalize_adapter_message_envelope(payload, actor_source=actor_source)

    def message_payload(self, envelope: GatewayMessageEnvelope) -> dict[str, Any]:
        return gateway_message_payload(envelope)

    def task_state(self, record: Mapping[str, Any]) -> GatewayTaskStateView:
        return gateway_task_state_view(record)

    def task_record_view(self, record: Mapping[str, Any]) -> dict[str, Any]:
        return gateway_task_record_view(record)

    def artifact_manifest(self, artifact_dir: str | Path) -> dict[str, Any]:
        return artifact_manifest(artifact_dir)

    def artifact_views(self, record: Mapping[str, Any]) -> tuple[GatewayArtifactView, ...]:
        return artifact_views(record)

    def sanitize_task_record(self, record: Mapping[str, Any]) -> dict[str, Any]:
        return sanitize_task_record(record)
...
def normalize_message_envelope(payload: Mapping[str, Any]) -> GatewayMessageEnvelope:
    return normalize_adapter_message_envelope(payload)
...
def normalize_adapter_message_envelope(
    ...
    *,
    ...
    normalized_actor_source = _normalize_required_actor_source(actor_source)
    actor = _actor_ref(
        payload.get("actor"),
        default_source=normalized_actor_source or "adapter",
        required_source=normalized_actor_source,
    )
...
def _actor_ref(
    ...
    *,
    ...
    if required_source is not None and normalized_source != required_source:
        raise ValueError(f"actor.source must be {required_source}")
...
def _normalize_required_actor_source(value: str | None) -> str | None:
    if value is None:
        return None
    stripped = value.strip().lower()
    if not stripped:
        raise ValueError("actor_source must not be empty")
    return stripped
