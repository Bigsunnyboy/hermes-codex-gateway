...
Hermes Codex Gateway runs local Codex processes against user-selected
repositories. Treat it as an automation control plane, not as a secret store.

## Supported Use

- Keep `config.json`, Codex auth files, Feishu app credentials, API keys, and
...
- Use dedicated read-only or low-privilege credentials in projects that Codex can
  access.
